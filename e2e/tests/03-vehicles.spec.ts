...
import { loginAs, uniqueSuffix, ADMIN_EMAIL, ADMIN_PASSWORD } from './helpers';

// ============================================================
//  Testes E2E — CRUD de Veículos (cliente)
//  Fluxo completo: criar → listar → editar → excluir
// ============================================================

// Credenciais de um cliente de teste (criado dinamicamente)
...
test.describe('CRUD de Veículos', () => {
  ...
  // Garante que o cliente de teste existe antes dos testes
  test.beforeAll(async ({ browser }) => {
    const page = await browser.newPage();
    await page.goto('/cadastro');

    // Tenta cadastrar — ignora erro se já existir
    try {
  ...
  test('sucesso — cadastrar novo veículo', async ({ page }) => {
    ...
    // Veículo deve aparecer na listagem
  ...
  test('sucesso — editar veículo existente', async ({ page }) => {
    ...
    // Primeiro cria um veículo para editar
  ...
  test('falha — cadastrar veículo com placa duplicada exibe erro', async ({ page }) => {
    ...
    await expect(
      page.getByText(/placa.*cadastrada|placa.*exist|já cadastrad/i).first()
    ).toBeVisible();
  });
  ...
  test('falha — cadastrar veículo sem campos obrigatórios exibe erro', async ({ page }) => {
    ...
    await expect(
      page.getByText(/obrigatório|preencha|campo.*requerido/i).first()
    ).toBeVisible();
  });
