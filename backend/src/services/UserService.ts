...
export class UserService {
  async criar(nome: string, email: string, senha: string, cpf: string) {
    ...
    const dominioExiste = process.env.SKIP_EMAIL_DOMAIN_CHECK === 'true' ? true : await validateEmailDomain(email);
    ...
    const usuario = await UsuarioModel.create({ nome, email, senha: senhaCriptografada, cpf });
  ...
  async atualizar(usuarioId: string, solicitanteId: string, dados: DadosAtualizacaoUsuario) {
    ...
    await UsuarioModel.update(dadosAtualizados, { where: { id: usuarioId } });

    return UsuarioModel.findByPk(usuarioId, {
  ...
  async listarTodos(consulta: ConsultaPaginacao) {
    ...
    const { rows: dados, count: total } = await UsuarioModel.findAndCountAll({
