...
export class AgendamentoService {
  async criar(dados: DadosCriacaoAgendamento) {
    const servico = await ServicoModel.findByPk(dados.servicoId);
  ...
  async listarPorUsuario(usuarioId: string, consulta: ConsultaPaginacao) {
    ...
    const { rows: dadosLista, count: total } = await AgendamentoModel.findAndCountAll({
      where: { usuarioId },
      offset: skip,
      limit: pageLimit,
      include: [
        { model: VeiculoModel, as: 'veiculo' },
        { model: ServicoModel, as: 'servico' },
      ],
  ...
  async listarTodos(consulta: ConsultaPaginacao) {
    ...
    const { rows: dadosLista, count: total } = await AgendamentoModel.findAndCountAll({
      ...
      limit: pageLimit,
      include: [
        { model: UsuarioModel, as: 'usuario', attributes: ['nome', 'email'] },
        { model: VeiculoModel, as: 'veiculo' },
        { model: ServicoModel, as: 'servico' },
      ],
      order: [['agendadoPara', 'DESC']],
  ...
  async atualizarStatus(id: string, status: StatusAgendamento, isAdmin: boolean, usuarioId: string) {
    ...
    const agendamento = await AgendamentoModel.findOne({ where: condicao });
  ...
  private async verificarConflito(inicio: Date, fim: Date): Promise<void> {
    type AgendamentoComServico = AgendamentoModel & { servico: { duracao: number } };

    const ativos = await AgendamentoModel.findAll({
      where: { status: { [Op.in]: ['pendente', 'em_andamento'] } },
      include: [{ model: ServicoModel, as: 'servico', attributes: ['duracao'] }],
