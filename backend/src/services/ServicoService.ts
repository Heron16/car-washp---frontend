...
export class ServicoService {
  async criar(nome: string, descricao: string, preco: number, duracao: number, tiposVeiculo: TipoVeiculo[]) {
    ...
    const servico = await ServicoModel.create({ nome, descricao, preco, duracao, tiposVeiculo: tiposVeiculo.join(',') });
  ...
  async listar(consulta: ConsultaPaginacao) {
    ...
    const { rows, count: total } = await ServicoModel.findAndCountAll({
      where: { ativo: true },
      offset: skip,
      limit: pageLimit,
      order: [['criadoEm', 'DESC']],
  ...
  private formatar(s: ServicoModel) {
    type ServicoPlano = {
