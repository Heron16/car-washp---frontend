...
import { Veiculo, ResultadoPaginado, TipoVeiculo } from '../../types';
...
export function AdminVehiclesPage() {
  ...
  const buscarVeiculos = () => {
    api.get<ResultadoPaginado<Veiculo>>(`/vehicles?pagina=${page}&limite=${limit}`)
  ...
  const handleExcluir = async (id: string) => {
    ...
    try { await api.delete(`/vehicles/${id}`); toast.success('Veículo removido'); buscarVeiculos(); }
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-400">{v.cor}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4 font-mono text-gray-700">{v.placa}</td>
                  <td className="px-6 py-4 text-gray-500">{labelsTipo[v.tipo]}</td>
                  <td className="px-6 py-4 text-gray-500">{v.ano}</td>
                  <td className="px-6 py-4">
