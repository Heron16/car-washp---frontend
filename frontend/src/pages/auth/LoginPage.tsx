import { useState, FormEvent } from 'react';
import { Link, useNavigate } from 'react-router-dom';
import { useAuth } from '../../contexts/AuthContext';
import { Input } from '../../components/common/Input';
import { Button } from '../../components/common/Button';
import { validateEmail } from '../../utils/validators';
import { ErroApi } from '../../types';

export function LoginPage() {
  const { login } = useAuth();
  const navigate = useNavigate();
  const [email, setEmail] = useState('');
  const [senha, setSenha] = useState('');
  const [erroLogin, setErroLogin] = useState('');
  const [errosCampos, setErrosCampos] = useState<{ email?: string; senha?: string }>({});
  const [loading, setLoading] = useState(false);

  const validar = () => {
    const erros: { email?: string; senha?: string } = {};
    if (!email) erros.email = 'E-mail é obrigatório';
    else if (!validateEmail(email)) erros.email = 'Formato de e-mail inválido';
    if (!senha) erros.senha = 'Senha é obrigatória';
    setErrosCampos(erros);
    return Object.keys(erros).length === 0;
  };

  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    setErroLogin('');
    if (!validar()) return;
    setLoading(true);
    try {
      const usuarioLogado = await login(email, senha);
      navigate(usuarioLogado.perfil === 'admin' ? '/admin' : '/dashboard');
    } catch (err: unknown) {
      const msg = (err as ErroApi)?.response?.data?.mensagem || 'E-mail ou senha inválida';
      setErroLogin(msg);
    } finally {
      setLoading(false);
    }
  };

  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-900 via-blue-800 to-cyan-700 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <div className="w-16 h-16 bg-white/20 backdrop-blur rounded-2xl flex items-center justify-center mx-auto mb-4">
            <span className="text-3xl">🚿</span>
          </div>
          <h1 className="text-3xl font-bold text-white">AquaWash</h1>
          <p className="text-blue-200 mt-1">Seu veículo merece o melhor</p>
        </div>

        <div className="bg-white rounded-2xl shadow-2xl p-8">
          <h2 className="text-xl font-semibold text-gray-900 mb-6">Entrar na sua conta</h2>

          <form onSubmit={handleSubmit} className="space-y-4">
            <Input
              label="E-mail"
              type="email"
              placeholder="seu@email.com"
              value={email}
              onChange={(e) => { setEmail(e.target.value); setErroLogin(''); if (errosCampos.email) setErrosCampos((p) => ({ ...p, email: undefined })); }}
              error={errosCampos.email}
            />
            <Input
              label="Senha"
              type="password"
              placeholder="••••••••"
              value={senha}
              onChange={(e) => { setSenha(e.target.value); setErroLogin(''); if (errosCampos.senha) setErrosCampos((p) => ({ ...p, senha: undefined })); }}
              error={errosCampos.senha}
            />

            {erroLogin && (
              <div className="flex items-center gap-2 p-3 bg-red-50 border border-red-300 rounded-lg">
                <span className="text-red-500 text-base">⚠️</span>
                <p className="text-sm text-red-700 font-medium">{erroLogin}</p>
              </div>
            )}

            <Button type="submit" loading={loading} className="w-full" size="lg">Entrar</Button>
          </form>

          <p className="text-center text-sm text-gray-500 mt-6">
            Não tem conta?{' '}
            <Link to="/cadastro" className="text-blue-600 font-medium hover:underline">Cadastre-se</Link>
          </p>
        </div>
      </div>
    </div>
  );
}
