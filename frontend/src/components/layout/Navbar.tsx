          ...
          <div className="flex items-center gap-4">
            {!estaAutenticado ? (
              <>
                ...
                <Link to="/cadastro" className="bg-blue-600 text-white text-sm px-4 py-2 rounded-lg hover:bg-blue-700 transition font-medium">Cadastrar</Link>
              </>
            ) : (
              <>
                <Link
                  to={usuario?.perfil === 'admin' ? '/admin' : '/dashboard'}
                  className="text-sm text-gray-600 hover:text-blue-600 transition font-medium"
                >
                  {usuario?.perfil === 'admin' ? 'Painel Admin' : 'Minha Área'}
                </Link>
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center">
                    <span className="text-blue-600 text-sm font-semibold">{usuario?.nome[0].toUpperCase()}</span>
                  </div>
